...
function Modal({ modalRef, playlistVideo, setShowModal }) {
  ...
  const deleteVideoDataInPlaylist = async (
    authState,
    playlistId,
    playlistVideo,
    playlistDispatch
    ...
    if (authState.token) {
      try {
        const res = await axios.delete(
          `/api/user/playlists/${playlistId}/${playlistVideo._id}`,
  ...
  return (
    <div ref={modalRef} className="playlist-conatainer">
      <div className="playlist-box">
        <div>
          <h1 className="playlist-header">Create New Playlist</h1>{" "}
          <IoClose
        ...
        >
          <div className="playlist-text">
            ...
            <input
              ...
              maxLength={12}
              onChange={(e) =>
                setPlaylistName((pre) => ({ ...pre, title: e.target.value }))
              }
              ...
              <div key={playlistId} className="label-box">
                <label htmlFor={playlistId}>
                  <input
                    ...
                    onClick={() => {
                      onCheckVideoHandler(playlistId, videos, playlistVideo);
                    }}
                  />
                  {title}
                </label>
          ...
          <div>
            <button type="Submit" className="playlist-btn">
              Create playlist
            </button>
          </div>
        </form>
